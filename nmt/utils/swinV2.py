...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
            if layer_count == len(self.layers) - node_layer:
                nodes = self.norm(x)
            elif layer_count == len(self.layers) - edge_layer:
                edges = self.norm(x)
        ...
        nodes = nodes.reshape(nodes.shape[0], nodes.shape[1], nodes_patchSize, nodes_patchSize)
        edges = edges.reshape(edges.shape[0], edges.shape[1], edges_patchSize, edges_patchSize)

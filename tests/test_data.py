...
from rational_cnn import PairDataset, ValidPairDataset
...
def test_pair_dataset():
    ...
    assert dataset.__repr__() == (
        'PairDataset([Data(x=[10, 16], edge_index=[2, 30]), '
        'Data(x=[10, 16], edge_index=[2, 30])], ['
        'Data(x=[10, 16], edge_index=[2, 30]), '
        'Data(x=[10, 16], edge_index=[2, 30])], sample=True)')
    ...
    assert dataset.__repr__() == (
        ...
        'Data(x=[10, 16], edge_index=[2, 30])], ['
        'Data(x=[10, 16], edge_index=[2, 30]), '
        'Data(x=[10, 16], edge_index=[2, 30])], sample=False)')
...
def test_valid_pair_dataset():
    ...
    assert dataset.__repr__() == (
        'ValidPairDataset([Data(x=[10, 16], edge_index=[2, 30], y=[10]), '
        'Data(x=[10, 16], edge_index=[2, 30], y=[10])], ['
        'Data(x=[10, 16], edge_index=[2, 30], y=[10]), '
        'Data(x=[10, 16], edge_index=[2, 30], y=[10])], sample=True)')
    assert len(dataset) == 2
    pair = dataset[0]
    assert len(pair) == 5
    assert torch.allclose(pair.x_s, x)
    ...
    assert dataset.__repr__() == (
        ...
        'Data(x=[10, 16], edge_index=[2, 30], y=[10])], ['
        'Data(x=[10, 16], edge_index=[2, 30], y=[10]), '
        'Data(x=[10, 16], edge_index=[2, 30], y=[10])], sample=False)')
    assert len(dataset) == 4
    pair = dataset[0]
    assert len(pair) == 5
    assert torch.allclose(pair.x_s, x)

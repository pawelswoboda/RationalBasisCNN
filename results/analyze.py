...
line. N-Caltech101 and N-Cars (AEGNN): same format, classification accuracy.
...
def summarize(subdir, title, tests, best_label='best acc'):
    r"""Per-seed 'Final test accuracy: x (best y)' logs (FAUST vertex
    accuracy, N-Caltech101 / N-Cars classification accuracy) -> table +
    t-tests.

    The producers disagree on what the second number is: experiments/faust.py
    and experiments/pascal_voc.py take a maximum over test evaluations, while
    experiments/ncaltech101.py records the test accuracy at the best-validation
    epoch. They must not share a column header, so `best_label` names it."""
    print('\n' + '=' * 78)
    print(title)
    ...
    print(f"{'config':26s} {'params':>8s} " + (f"{'conv':>7s} " if has_conv
          else '') + f"{'seeds':>5s} {'final acc':>14s} {best_label:>14s}  "
          "per-seed")
